  ...
  Predictor: class {
    ...
    initPredictions() {
      ...
      this.helpers = (this.helpers || {
         ...
         * @memberof Koviko.Predictor#helpers
         */
        getTeamCombat: (r, k) => h.getSelfCombat(r, k) + g.getSkillLevelFromExp(k.restoration) * (2 + (r.team || 0))/ 2 + g.getSkillLevelFromExp(k.combat) * (r.team || 0) / 2 * h.getGuildRankBonus(r.adventures || 0),
      });
      ...
      const predictions = {
        ...
        'Looping Potion': { affected: ['herbs', 'lpotions'], effect: (r, k) => {
          if ( r.herbs >= 200 ) {
            (r.herbs -= 200, r.lpotions++, k.alchemy += 100)
          }
        }},
        'Check Walls': {},
    ...
    update(actions, container, isDebug) {
      ...
      if ( !((snapshots.currProgress.attributes["Small Dungeon"].value) === undefined) ) { dungeonSoulStones += snapshots.currProgress.attributes["Small Dungeon"].value };
      if ( !((snapshots.currProgress.attributes["Large Dungeon"].value) === undefined) ) { dungeonSoulStones += snapshots.currProgress.attributes["Large Dungeon"].value * 10 };
